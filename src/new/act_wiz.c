...
void do_ostat( CHAR_DATA *ch, char *argument )
{
    ...
    if (obj->weapon != NULL)
    {
	sprintf(buf,"Weapon type: %s  damage message: %s\n\r",
		weapon_table[obj->weapon->type].name,
		attack_table[obj->weapon->message].name);
	send_to_char(buf,ch);

	sprintf(buf,"Weapon flags: %s\n\r",
		weapon_bit_name(obj->weapon->flags));
	send_to_char(buf,ch);

	sprintf(buf,"Weapon is %s%d to hit, damage %dd%d%s%d\n\r",
		(obj->weapon->hit < 0) ? "-" : "+",
		obj->weapon->hit,
		obj->weapon->damage[DICE_NUMBER],
		obj->weapon->damage[DICE_TYPE],
		(obj->weapon->damage[DICE_BONUS] < 0) ? "-" : "+",
		obj->weapon->damage[DICE_BONUS]);
	send_to_char(buf,ch);

	if (obj->weapon->slaying != NULL)
	{
	    sprintf(buf,"Weapon slays %s\n\r",obj->weapon->slaying);
	    send_to_char(buf,ch);
	}
    }
    ...
    {
	sprintf(buf,
		"This item holds %d pounds (max. per object is %d pounds)\n\r",
		obj->cont->volume,	obj->cont->limit);
	send_to_char(buf,ch);

	if (obj->cont->key > 0)
	{
	    sprintf(buf,"Key vnum is %d\n\r",obj->cont->key);
	    send_to_char(buf,ch);
	}

	if (obj->cont->closed != 0)
	{
	    sprintf(buf,"The object is %s\n\r", 
	 	cont_bit_name(obj->cont->closed)); 
	    send_to_char(buf,ch);
	}
    }

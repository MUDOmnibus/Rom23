...
{   
	...
	if ( !IS_NPC(ch) && ch->level < LEVEL_IMMORTAL )
	{
	    ...
	    {
		if ( --obj->light->duration == 0 && ch->in_room != NULL )
		{
		    --ch->in_room->light;
	 	...
	 	else if ( obj->light->duration <= 5 && ch->in_room != NULL)
		    act("$p flickers warningly.",ch,obj,NULL,TO_CHAR);
	    }
...
{   
	...
	if (IS_FOUNTAIN(obj) || IS_POTION(obj))
	    message = "$p dries up.";
	else if (IS_CORPSE(obj))
	    message = "$p decays into dust.";
	else if (IS_FOOD(obj))
	    message = "$p is covered by mold and rots away.";
	else 
	...
	&&      ( rch = obj->in_room->people ) != NULL )
	{
        ...
        if (IS_CORPSE_PC(obj) && obj->contains)
	{   /* save the contents */

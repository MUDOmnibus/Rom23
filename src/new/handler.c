...
int apply_ac( OBJ_DATA *obj, int iWear, int type )
{
    ...
    switch ( iWear )
    {
    case WEAR_BODY:	return 3 * obj->armor->ac[type];
    case WEAR_HEAD:	return 2 * obj->armor->ac[type];
    case WEAR_LEGS:	return 2 * obj->armor->ac[type];
    case WEAR_FEET:	return     obj->armor->ac[type];
    case WEAR_HANDS:	return     obj->armor->ac[type];
    case WEAR_ARMS:	return     obj->armor->ac[type];
    case WEAR_SHIELD:	return     obj->armor->ac[type];
    case WEAR_FINGER_L:	return     obj->armor->ac[type];
    case WEAR_FINGER_R: return     obj->armor->ac[type];
    case WEAR_NECK_1:	return     obj->armor->ac[type];
    case WEAR_NECK_2:	return     obj->armor->ac[type];
    case WEAR_ABOUT:	return     obj->armor->ac[type];
    case WEAR_WAIST:	return     obj->armor->ac[type];
    case WEAR_WRIST_L:	return     obj->armor->ac[type];
    case WEAR_WRIST_R:	return     obj->armor->ac[type];
    case WEAR_HOLD:	return     obj->armor->ac[type];
    }
...
int get_obj_number( OBJ_DATA *obj )
{
    ...
    if ( obj->type == ITEM_CONTAINER || IS_MONEY(obj))
        number = 0;
    else
        number = 1;
...
bool can_see_obj( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if ( IS_LIGHT(obj) && obj->light->duration != 0 )
	return TRUE;
...
char *item_type_name( OBJ_DATA *obj )
{
    ...
    buf[0] = '\0';
    if (IS_LIGHT(obj))		strcat(buf," light");
    if (IS_SCROLL(obj))		strcat(buf," scroll");
    if (IS_WAND(obj))		strcat(buf," wand");
    if (IS_STAFF(obj))		strcat(buf," staff");
    if (IS_WEAPON(obj))		strcat(buf," weapon");
    if (IS_TREASURE(obj))	strcat(buf," treasure");
    if (IS_ARMOR(obj))		strcat(buf," armor");
    if (IS_POTION(obj))		strcat(buf," potion");
    if (IS_FURNITURE(obj))	strcat(buf," furniture");
    if (IS_TRASH(obj))		strcat(buf," trash");
    if (IS_CONT(obj))		strcat(buf," container");
    if (IS_DCON(obj))		strcat(buf," drink");
    if (IS_KEY(obj))		strcat(buf," key");
    if (IS_FOOD(obj))		strcat(buf," food");
    if (IS_MONEY(obj))		strcat(buf," money");
    if (IS_BOAT(obj))		strcat(buf," boat");
    if (IS_CORPSE(obj))		strcat(buf," corpse");
    if (IS_FOUNTAIN(obj))	strcat(buf," fountain");
    if (IS_PILL(obj))		strcat(buf," pill");
    if (IS_MAP(obj))		strcat(buf," map");
...
char *cont_bit_name(int cont_flags)
{
    ...
    {
	REMOVE_BIT(counter,CONT_CLOSEABLE);
	strcat(buf," closeable");
    }

    if (cont_flags & CONT_PICKPROOF)
    {
	REMOVE_BIT(counter,CONT_PICKPROOF); 
	if (counter == 0 && buf[0] != '\0')
	    strcat(buf," and");
	else if (buf[0] != '\0')
	    strcat(buf,",");
    ...
    {
	REMOVE_BIT(counter,CONT_CLOSED);
	if (counter == 0 && buf[0] != '\0') 
	    strcat(buf," and");
	else if (buf[0] != '\0')
	    strcat(buf,",");
    ...
    {
	REMOVE_BIT(counter,CONT_LOCKED);
	if (counter == 0 && buf[0] != '\0')
	    strcat(buf," and");
	else if (buf[0] != '\0')
	    strcat(buf,",");

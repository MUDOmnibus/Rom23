...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
	...
	ch->gold += obj->cost;
	  ...
	  if ( members > 1 && obj->cost > 1)
	  {
	    sprintf(buffer,"%d",obj->cost);
	    do_split(ch,buffer);	
	  }
        }
...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_CORPSE(container) && !IS_CONT(container))
	{
...
void do_put( CHAR_DATA *ch, char *argument )
{
	...
	if ( get_obj_weight( obj ) + get_obj_weight( container )
	     > container->cont->volume 
	||   get_obj_weight(obj) > container->cont->limit )
	{
...
void do_fill( CHAR_DATA *ch, char *argument )
{
    ...
    if ( obj->cont->volume != 0 && obj->cont->liquid != 0 )
    {
	send_to_char( "There is already another liquid in it.\n\r", ch );
	return;
    }

    if ( obj->cont->volume >= obj->cont->limit )
    {
...
void do_drink( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_POTION(obj) && !IS_DCON(obj) && !IS_FOUNTAIN(obj))
    {
    ...
    {
	if ( obj->cont->volume <= 0 )
	{
	    send_to_char( "It is already empty.\n\r", ch );
	    return;
	}

	if ( ( liquid = obj->cont->liquid ) >= LIQ_MAX )
	{
	    bug( "Do_drink: bad liquid number %d.", liquid );
	    liquid = obj->cont->liquid = 0;
	}
	...
	amount = UMIN(amount, obj->cont->volume);
	...
	if (!IS_DCON(obj) && !IS_FOUNTAIN(obj))
	{
        ...
        for (i = 0; i < 4; i++)
	    obj_cast_spell(obj->spells->spell[i],obj->spells->level,ch,ch,NULL);
	WAIT_STATE(ch,obj->spells->delay * PULSE_VIOLENCE);
	
	if ((!IS_DCON(obj) || obj->cont->volume == 0) && !IS_FOUNTAIN(obj)) 
	    extract_obj(obj);
    }
...
void do_eat( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_NPC(ch))
	{
        ...
        if ( obj->food->poison )
        {
            ...
            af.type      = gsn_poison;
            af.level     = number_fuzzy(obj->food->bite);
	...
	obj->food->full -= obj->food->bite;
	if (obj->food->bite < obj->food->full)
	    obj->food->bite = obj->food->full;
	if ( !IS_NPC(ch) && condition == 0 
	&&   ch->pcdata->condition[COND_FULL] > 0 )
	    send_to_char( "You are no longer hungry.\n\r", ch );
	else if ( !IS_NPC(ch) && ch->pcdata->condition[COND_FULL] > 40 )
	    send_to_char( "You are full.\n\r", ch );
    }
    ...
    if (!IS_FOOD(obj) || obj->food->full <= 0)
    	extract_obj( obj );
...
void do_recite( CHAR_DATA *ch, char *argument )
{
	...
	for (i = 0; i < 4; i++)
    	    obj_cast_spell( scroll->spells->spell[i], 
		            scroll->spells->level,ch,victim,obj);
	WAIT_STATE(ch,obj->spells->delay * PULSE_VIOLENCE);
...
void do_use( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_STAFF(obj) && IS_WAND(obj))
    {
	send_to_char("You can't use that.\n\r",ch);
	return;
    }

    if ((sn = obj->use->spell) < 0
    ||	 sn >= MAX_SKILL
    ||	 skill_table[sn].spell_fun == 0)
    {
	bug("Do_use: bad sn %d.",sn);
	return;
    }

    WAIT_STATE(ch,obj->use->delay * PULSE_VIOLENCE);

    if (obj->use->charges <= 0)
    {
	act("$n invokes $p...and nothing happens.",ch,obj,NULL,TO_ROOM);
	send_to_char("You seem to be out of charges.\n\r",ch);
	if (obj->type == ITEM_WAND || obj->type == ITEM_STAFF)
	{
	    act("$p shakes violently and expodes.",ch,obj,NULL,TO_ROOM);
	    act("$p shakes violently and explodes.",ch,obj,NULL,TO_ROOM);
	    extract_obj(obj);
	}
	else  /* remove use attribute */
	    free_use(obj);
    }

    if (IS_STAFF(obj))
    {
	act("$n brandishes $p.",ch,obj,NULL,TO_ROOM);
	act("You brandish $p.",ch,obj,NULL,TO_CHAR);
	if (ch->level < obj->level 
	||  number_percent() >= 20 + get_skill(ch,gsn_staves) * 4/5)
	{
	    act("...and nothing happens.",ch,NULL,NULL,TO_ROOM);
	    act("You fail to invoke $p.",ch,obj,NULL,TO_CHAR);
	    check_improve(ch,gsn_staves,FALSE,2);
	    obj->use->charges--;
	    return;
	}
	
	else for (victim = ch->in_room->people; victim != NULL; victim = v_next)
	{
		...
		case TAR_IGNORE:  case TAR_CHAR_SELF:
		    if (victim != ch)
			continue;
		break;
		...
		case TAR_CHAR_DEFENSIVE:
		    if (IS_NPC(ch) ? !IS_NPC(victim) : IS_NPC(victim))
			continue;
		break;
	    }

	    obj_cast_spell(sn,obj->use->level,ch,victim,NULL);
	}
	...
	else
	{
	    ...
	    target = get_obj_here(ch,argument);
	}

	if (victim != NULL)
	{
	    act("$n zaps $N with $p.",ch,obj,victim,TO_NOTVICT);
	    act("You zap $N with $p.",ch,obj,victim,TO_CHAR);
	    act("$n zaps you with $p.",ch,obj,victim,TO_VICT);
	}

	else if (target != NULL)
	{
	    act("$n zaps $P with $p.",ch,obj,target,TO_ROOM);
	    act("You zap $P with $p.",ch,obj,target,TO_CHAR);
	}
	...
	{
	    obj_cast_spell(sn,obj->use->level,ch,victim,target);
...
int get_cost( CHAR_DATA *keeper, OBJ_DATA *obj, bool fBuy )
{
	...
	{
	    if ( IS_SET(obj->type,pShop->buy_type[itype]))
	    {
    ...
    if ( IS_STAFF(obj) || IS_WAND(obj))
	cost -= (cost * obj->use->charges / obj->use->max) / 2;

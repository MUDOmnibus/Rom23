...
void spell_create_water( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( obj->cont->liquid != LIQ_WATER && obj->cont->volume != 0 )
    {
    ...
    water = UMIN(
		...
		obj->cont->limit - obj->cont->volume
		);
  
    if ( water > 0 )
    {
	obj->cont->liquid = LIQ_WATER;
	obj->cont->volume += water;
...
void spell_detect_poison( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	if (obj->cont->poison)
	    send_to_char("This brew has a sinister look about it.\n\r",ch);
	else
	    send_to_char("It looks quite drinkable.\n\r",ch);
    }
    
    if ( IS_FOOD(obj))
    {
	if (obj->food->poison)
	    send_to_char("I wouldn't eat it if I were you.\n\r",ch);
	else
	    send_to_char("It looks quite edible.\n\r",ch);
    }
    else
    {
...
void spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	obj_lose = obj_next )
	{
	    ...
	    else if (IS_POTION(obj_lose) || IS_DCON(obj_lose))
		msg = "$p bubbles and boils!";
	    else if (IS_SCROLL(obj_lose))
		msg = "$p crackles and burns!";
	    else if (IS_STAFF(obj_lose) || IS_WAND(obj_lose))
		msg = "$p smokes and chars!";
	    else if (IS_FOOD(obj_lose) || IS_PILL(obj_lose))
...
void spell_frost_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	obj_lose = obj_next )
	{
	    ...
	    if (IS_DCON(obj_lose) || IS_POTION(obj_lose))

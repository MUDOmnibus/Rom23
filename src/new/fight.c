...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if (dt < TYPE_HIT)
    	if (wield != NULL)
    	    dam_type = attack_table[wield->weapon->message].damage;
    	else
    ...
    if (wield!= NULL && IS_WEAPON(wield))
	thac0 -= wield->weapon->hit * skill/100;
	...
	if ( wield != NULL && IS_WEAPON(wield))
	{
	    dam = dice(wield->weapon->damage[DICE_NUMBER],
		       wield->weapon->damage[DICE_TYPE])
		  +    wield->weapon->damage[DICE_BONUS];
	    dam *= skill / 100;
    ...
    if ( dt == gsn_backstab && wield != NULL) 
    	if ( wield->weapon->type != WEAPON_DAGGER )
	    dam *= 2 + ch->level / 10; 
	else 
	    dam *= 2 + ch->level / 8;
...
void make_corpse( CHAR_DATA *ch )
{
	...
	obj_from_char( obj );
	if (obj->type == ITEM_POTION)
	    obj->timer = number_range(500,1000);
	if (obj->type == ITEM_SCROLL)
...
void death_cry( CHAR_DATA *ch )
{
	...
	if (IS_FOOD(obj))
	{
		...
		obj->food->poison = TRUE;
	    else if (!IS_SET(ch->form,FORM_EDIBLE))
	    {
		REMOVE_BIT(obj->type,ITEM_FOOD);
		free_food(obj);
	    }
	}
